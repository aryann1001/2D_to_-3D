...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="",
        help="path to checkpoint to load lift3d model state from"
    )
    parser.add_argument(
        "--finetune_from",
        type=str,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.lift3d_ckpt == "":
            rank_zero_print(f"Attempting to load state from {opt.lift3d_ckpt}")
            org_state = torch.load(opt.lift3d_ckpt, map_location="cpu")
            if "state_dict" in org_state:
                rank_zero_print(f"Found nested key 'state_dict' in checkpoint, loading this instead")
                org_state = org_state["state_dict"]
            lift3d_state = {}
            for k, v in org_state.items():
                if k.startswith("model."):
                    new_key = k.replace("model.", "Lift3D.")
                    lift3d_state[new_key] = v
            m, u = model.load_state_dict(lift3d_state, strict=False)
        ...
        if not opt.finetune_from == "":
            ...
            # Check if we use control_model. If so, we need to copy the weight.
            control_flag = False
            ...
            if control_flag:
                ...
                for k in scratch_dict.keys():
                    ...
                    if is_control:
                        ...
                        copy_k = k
                    if copy_k in old_state:
                        target_dict[k] = old_state[copy_k].clone()
                    else:
                        target_dict[k] = scratch_dict[k].clone()
                        # print(f'These weights are newly added: {k}')
                old_state = target_dict
            ...
            #             input_weight.zero_()
            #             input_weight[:, :8, :, :].copy_(old_state[input_key])
        ...
        if "modelcheckpoint" in lightning_config:
            modelckpt_cfg_step = lightning_config.modelcheckpoint
        else:
            modelckpt_cfg_step =  OmegaConf.create()
        modelckpt_cfg_repeat = OmegaConf.merge(modelckpt_cfg_repeat, modelckpt_cfg_step)
        modelckpt_cfg = OmegaConf.merge(modelckpt_cfg, modelckpt_cfg_step)
        rank_zero_print(f"Merged modelckpt-cfg: \n{modelckpt_cfg_repeat}")
        if version.parse(pl.__version__) < version.parse('1.4.0'):
            trainer_kwargs["modelckpt_cfg"] = instantiate_from_config(modelckpt_cfg)
            trainer_kwargs["modelckpt_cfg_repeat"] = instantiate_from_config(modelckpt_cfg_repeat)
        ...
        if version.parse(pl.__version__) >= version.parse('1.4.0'):
            default_callbacks_cfg.update({'modelckpt_cfg': modelckpt_cfg})
            default_callbacks_cfg.update({'modelckpt_cfg_repeat': modelckpt_cfg_repeat})
