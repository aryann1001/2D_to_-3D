...
def load_pretrain_stable_diffusion(new_model, finetune_from, lift3d_ckpt):
    ...
    for name, _ in model.named_modules():
        if 'model.control_model' in name:
            control_flag = True
            break
    if control_flag:
        ...
        for k in scratch_dict.keys():
            is_control, name = get_node_name(k, 'model.control_')
            if is_control:
    ...
    rank_zero_print(f"Attempting to load lift3d state from {lift3d_ckpt}")
    org_state = torch.load(lift3d_ckpt, map_location="cpu")
    ...
    old_state.update(lift3d_state)
    m, u = new_model.load_state_dict(old_state, strict=False)
    if len(m) > 0:
...
if __name__ == "__main__":
    ...
    if opt.finetune_from != "":
        load_pretrain_stable_diffusion(model, opt.finetune_from, opt.lift3d_ckpt)
    ...
    default_callbacks_cfg = {
        "setup_callback": {
            "target": "train_syncdreamer_tri.SetupCallback",
            "params": {"resume": opt.resume, "logdir": logdir, "ckptdir": ckptdir, "cfgdir": cfgdir, "config": config}
        },
        "learning_rate_logger": {
            "target": "train_syncdreamer_tri.LearningRateMonitor",
            "params": {"logging_interval": "step"}
        },
        "cuda_callback": {"target": "train_syncdreamer_tri.CUDACallback"},
    }
    ...
    # print(trainer_config["gpus"])
    trainer_config["accelerator"] = "ddp"
    # exit(0)
    trainer = Trainer.from_argparse_args(args=argparse.Namespace(), **trainer_config, 
                                        plugins=[DDPPlugin(find_unused_parameters=False)], logger=logger, callbacks=callbacks)
